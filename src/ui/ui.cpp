...
// This is mostly a boiler-plate code directly taken from the example
// provided by Dear ImGui official repository. Some portion has been
// modified to match with the project dependencies and decouple some
// codes for a clean implementation of the actual necessary widgets.
...
static void check_vk_result(VkResult err)
{
    if (err == VK_SUCCESS)
        return;
    fmt::println(
        stderr, "[vulkan] Error: VkResult = {}", static_cast<int>(err)
    );
    if (err < 0)
        abort();
}
...
)
{
    ...
    fmt::println(
        stderr, "[vulkan] Debug report from ObjectType: {}\nMessage: {}\n",
...
)
{
    ...
    if (res != VK_TRUE)
    {
        fmt::println(stderr, "Error no WSI support on physical device 0");
        exit(-1);
    }
    ...
    // fmt::println("[vulkan] Selected PresentMode = {}", static_cast<int>(wd->PresentMode));
...
static void FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
{
    ...
    VkResult err = vkAcquireNextImageKHR(
        g_Device, wd->Swapchain, UINT64_MAX, image_acquired_semaphore,
...
{
    // Wayland does not allow application to handle its own icons
    if (glfwGetPlatform() != GLFW_PLATFORM_WAYLAND)
    {
        auto icon_paths = GET_ICON_PATHS();
        constexpr auto icon_count = icon_paths.size();
        ...
        {
            icons[i].pixels = stbi_load(
                icon_paths[i].c_str(), &(icons[i].width), &(icons[i].height),
                nullptr, 4
            );
            if (!icons[i].pixels)
            {
...
int ui(TelemetryServer& server, int port_num)
{
    ...
    GLFWwindow* window = glfwCreateWindow(
        (int)(WINDOW_WIDTH * main_scale), (int)(WINDOW_HEIGHT * main_scale),
        WINDOW_TITLE, nullptr, nullptr
    );
    if (!glfwVulkanSupported())
    {
        fmt::println("GLFW: Vulkan Not Supported");
        return 1;
    }
